...
SLAMParams read_params(ros::NodeHandle nh) {
  ...
  nh.getParam("pnp_method", slam_params.pnp_method);
...
int main(int argc, char *argv[]) {
  ...
  int pnp_method_enum = read_pnp_method(slam_params.pnp_method);
  ...
  for (unsigned int i = slam_params.initial_image_skip_num;
    ...
    if (slam_params.test_with_optimization) {
      ...
      //             front_end(last_orb_result, orb_res, good_matches,
      //             front_end_data,
    ...
    //         // TODO: BUG: pose should be relative to 10 frames before, not
    //         now. if (i% 10 == 0){
    //             add_point_cloud(pose, image, depth_image, cam_info,
    //             point_cloud,
    //                             slam_params);
    //             if (slam_params.pause_after_optimization) {
    //                 const std::string msg =
    //                     "Iteration " + std::to_string(i) + ", Please hit
    //                     enter pause";
    ...
    //             visualize_slam_results(optimized_poses, poses_pub,
    //             points_pub,
    //                                     point_cloud);
    //             }
    //       }
    //     last_orb_result = std::move(orb_res);
    //     front_end_data = FrontEndData{std::move(image),
