...
TEST(TestFeatureDetection, TestCircleOffset){
    ...
    for (auto r: horizontal_offsets){
        ...
        EXPECT_GE( (tl_abs.x+1) * (tl_abs.x+1) + (tl_abs.y+1) * (tl_abs.y+1), radius * radius);
    }
...
TEST(TestFeatureDetection, TestOrientation){
    // create a matrix with the second quadrant being +1. All other quadrants are 0. Then, calculate orientation
    ...
    std::vector<cv::KeyPoint> keypoints{cv::KeyPoint(cv::Point{radius+1, radius+1}, 1)};
...
TEST(TestFeatureDetection, TestDescriptorGeneration){
    ...
    std::cout << "Test passed: compute_descriptor function works as expected." << std::endl;
}
...
TEST(TestFeatureDetection, TestDescriptorBruteforce){
    ...
    // Expected match: desc1 should match with the second descriptor in desc2 (same data)
...
TEST(TestFeatureDetection, TestORB){
    ...
    ORBFeatureDetectionResult res1;
    // auto res1.image = cv::imread(image_path, cv::IMREAD_COLOR);
    // std::vector<KeyPoint> keypoints;
    // cv::Mat descriptors;
    // RgbdSlamRicoExercises::handwritten_orb(image1, keypoints, descriptors);
    
    // draw_feature_matches(const ORBFeatureDetectionResult &res1,
